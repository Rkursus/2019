...
Lae alla andmed *http://kodu.ut.ee/~annes/R/A.csv* ja õpime kasutama funktsioonie `read.csv()` ja `read.csv2()`.
...
- **Ülesanne 2**: Kontrolli, kas fail *A.csv*  on juba töökaustas olemas, kasuta selleks käsku `list.files()`. Tulemuseks peab olema töökaustas failide nimed. Faili saad vaadata ka aadressil *http://kodu.ut.ee/~annes/R/A.csv*  
...
Lae ÕIS-st kodutööde kaustast alla fail `dieet.csv`, paiguta see oma töökausta ja loe töölauale andmestik `dieet`. Tegu on teatavate dieetide mõju uuringuga. Uuringus osalejaid on kaalutud enne dieeti (`kaal1`) ja pärast dieeti (`kaal2`). Lisaks on teada iga inimese identifikaator ja dieedi tüüp.
 

```{r eval = F}
dieet <- read.csv2("dieet.csv", header = T)
